...
class BD09MercatorTilingScheme extends Cesium.WebMercatorTilingScheme {
    constructor(options) {
        super(options)
 ...
 * @param {String} options.url 影像服务地址
 * @param {String} [options.crs=WGS84] 坐标系
 * @param {String} [options.style=normal] 地图样式
 */
class BaiduImageryProvider {
    constructor(options = {}) {
        ...
        this._crs = options.crs || "WGS84"; // BD09
        if (this._crs === "WGS84") {
            const resolutions = [];
            for (let i = 0; i < 19; i++) {
                resolutions[i] = 256 * Math.pow(2, 18 - i);
            }
            this._tilingScheme = new BD09MercatorTilingScheme({
                resolutions,
    ...
    requestImage(x, y, level) {
        ...
        // Cesium.Resource直接解析百度地图服务的url会出现参数解析错误的问题
        // 不过Cesium.ImageryProvider.loadImage接受resource作为参数，因此这里直接通过自定义方法将url解析为resource。
        ...
        if (urls.length > 1) {
            ...
            for (let i = 0, len = params.length; i < len; ++i) {
                ...
                const val = params[i].substring(index + 1, params[i].length);
                query[key] = val;
            }
...
export { BD09MercatorTilingScheme, BaiduImageryProvider };
